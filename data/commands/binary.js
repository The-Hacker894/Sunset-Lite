...
module.exports.run = (client, message, args, data, game, announcement) => {
    ...
    var options = ['encode', 'decode', 'recover']
        ...
        if(!code) return message.channel.send('```' + boxen('Please specify whether or not you want to decode encode or recover text.', {padding: 1}) +'```')
        const modlog = message.guild.channels.find('name', 'mod-log');
    

    if(options.some(opt => code.includes(options))) {
        message.channel.send('```' + boxen('Please specify whether or not you want to decode encode or recover text.', {padding: 1}) +'```').then(message => {
